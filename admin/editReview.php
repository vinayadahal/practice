...
<div class="admin_edit_center">
    <h3>Edit Review</h3>
    <a href="<?php echo baseUrl; ?>admin/clientReview/" style="outline: 0px;" title="Back"><i class="glyphicon glyphicon-circle-arrow-left back_btn"></i></a>
    <form method="POST" action="<?php echo baseUrl . 'admin/'; ?>updateReview/">
        <input type="hidden" name="contentId" value="<?php echo $data['id']; ?>">
        <label for="heading"><h5>Quote:</h5></label> <input id="heading" type="text" class="form-control admin_editBox_override" name="heading" value="<?php echo $data['title']; ?>">
        <label for="bodyContact"><h5>Review:</h5></label> <textarea id="bodyContact" name="bodyContent"><?php echo $data['description']; ?></textarea>
        <input type="submit" value="Update" class="admin_edit_btn" />
    </form>
    <a href="<?php echo baseUrl ?>admin/clientReview/"><button class="admin_edit_btn">Cancel</button></a>
</div>
